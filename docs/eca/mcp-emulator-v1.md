...
| CONTRACT-SEL-CTX-1 | Resumen XML → `ui.tap(selector)` | Dos etiquetas repetidas solo se desambiguan con un ancestro semántico que deja un único destino. | Selector con `within`; XPath generado por el servidor; sin coordenadas ni XPath de entrada. |
